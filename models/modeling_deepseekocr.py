            ...
            for image, crop_shape in zip(images, images_spatial_crop):
                ...
                with torch.no_grad():
                    if torch.sum(patches).item() != 0:
                        ...
                        global_features = global_features.view(h, w, n_dim)
                        origin_global_features = global_features.clone()
                ...
                if images_in_this_batch:
                    ...
                    images_in_this_batch = images_in_this_batch.to(device=inputs_embeds.device, dtype=inputs_embeds.dtype)
                    inputs_embeds[idx].masked_scatter_(images_seq_mask[idx].unsqueeze(-1).to(device=inputs_embeds.device), images_in_this_batch)
                idx += 1
            
            self._origin_inputs_embeds = inputs_embeds
            self._origin_global_features = origin_global_features.to(device=inputs_embeds.device, dtype=inputs_embeds.dtype)
...
class DeepseekOCRForCausalLM(DeepseekV2ForCausalLM):
    ...
    def forward(
        ...
        if hidden_states.shape[1] != 1:
            self._origin_outputs_embeds = hidden_states
    ...
    def infer(self, tokenizer, prompt='', image_file='', output_path = '', base_size=1024, image_size=640, crop_mode=True, test_compress=False, save_results=False, eval_mode=False):
        ...
        origin_inputs_embeds = getattr(self.model, "_origin_inputs_embeds", None)
        mask = images_seq_mask.bool().to(origin_inputs_embeds.device)
        input_images_embeds = origin_inputs_embeds[0][mask]
        output_images_embeds = self._origin_outputs_embeds[0][mask]

        origin_global_features = getattr(self.model, "_origin_global_features", None)
        
        return input_images_embeds, output_images_embeds, origin_global_features